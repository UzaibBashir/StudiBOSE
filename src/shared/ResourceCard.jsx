export default function ResourceCard({ title, subtitle, subject, class: klass, year, driveUrl }) {
  return (
      ...
      <div className="flex items-center justify-between gap-2">
        <div>
          ...
          <div className="text-xs text-indigo-100/70">
            {[subtitle, subject && `Subject: ${subject}`, klass && `Class: ${klass}`, year && `Year: ${year}`]
              .filter(Boolean)
              .join(' • ')}
          </div>
        </div>
